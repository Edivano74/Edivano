...
function confirmation(id,data){return{id,price:'9,99 €',paidAt:data.paid_at||null,consentedAt:data.consented_at||null,immediatePerformance:data.consent_immediate_performance===true,withdrawalLossAcknowledged:data.consent_withdrawal_loss===true}}

export default async(req)=>{if(req.method!=='POST')return json({error:'Method not allowed'},405);try{const body=await req.json();const id=cleanText(body.id,80);const token=cleanText(body.token,120);const client=db(token);const {data,error}=await client.from('trip_requests').select('payment_status,paid_at,consented_at,consent_immediate_performance,consent_withdrawal_loss,full_plan_json,request_json,preview_json').eq('id',id).single();if(error)return json({ready:false,message:'Reiseplan nicht gefunden.'},404);if(data.payment_status!=='paid')return json({ready:false,message:'Zahlung wird noch bestätigt …'});if(data.full_plan_json)return json({ready:true,plan:data.full_plan_json,confirmation:confirmation(id,data)});const plan=await generatePlan(data.request_json,data.preview_json);const {error:updateError}=await client.from('trip_requests').update({full_plan_json:plan}).eq('id',id);if(updateError)console.error('plan save',updateError.message);return json({ready:true,plan,confirmation:confirmation(id,data)})}catch(e){console.error(e);return json({ready:false,message:'Plan konnte nicht geladen werden.'},500)}}
